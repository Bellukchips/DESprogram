...
public class DESProgram {
    ...
    public static void main(String[] args) {
        ...
        DESProgram cipher = new DESProgram();
    ...
    // permutasi inputan hexa 
    String permutation(int[] squence, String input) {
        String rslt = "";
        input = hexatoBin(input);
        for (int i = 0; i < squence.length; i++) {
            // menjumlahkan semua data yang berada pada list squence
            rslt += input.charAt(squence[i] - 1);
        }
        rslt = bintoHex(rslt);
        return rslt;
    }
    ...
    String xor(String a, String b) {
        long nA = Long.parseUnsignedLong(a, 16);
        long nB = Long.parseUnsignedLong(b, 16);

        nA = nA ^ nB;
        a = Long.toHexString(nA);
    ...
    String leftCircularShift(String input, int numBit) {
        int n = input.length() * 4;
        int prm[] = new int[n];
        for (int i = 0; i < n - 1; i++) {
            prm[i] = (i + 2);
